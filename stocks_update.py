...
def update_data_base_fast():
	...
	mf = pro.moneyflow_hsgt(start_date=one_year_before_str, end_date=today.strftime("%Y%m%d"))
...
def update_data_base_fast_simple():
	## Initialize database and setup tushare interface
	conn = sqlite3.connect('cn_stocks.db')
	pro = ts.pro_api()
	stocks_60 	= {}
	...
	widgets = [Bar('>'), ' ', ETA(), ' ', ReverseBar('<')]
	pbar = ProgressBar(widgets=widgets,maxval=len(all_stocks_list)).start()
	last_60 = pd.read_sql('select * from stocks_60_days;',conn)
	stocks_60_fast = {}
	i=0
...
def clean_up_today(date=datetime.datetime.today()):
	conn = sqlite3.connect('cn_stocks.db')
	cur = conn.cursor()
	cur.execute('DELETE FROM stocks_60_days WHERE date=\'{}\''.format(date.strftime('%Y-%m-%d')))
	cur.execute('DELETE FROM stocks_125_days WHERE date=\'{}\''.format(date.strftime('%Y-%m-%d')))
	conn.commit()
	conn.close()
