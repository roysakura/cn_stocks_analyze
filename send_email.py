...
def combine_qrcode(qrcode,template,dest):
    new_im = Image.open(template)
    qrcode = Image.open(qrcode)
    new_im.paste(qrcode,(575,190),0)
    new_im.save(dest)
...
def send(date=datetime.datetime.today()):
    ...
    for key,user in settings.SEND_USERS.items():
        footer_file = os.path.join(home,"Documents","{}_qr_footer.png".format(key))
        combine_qrcode(user['qrcode'],settings.IMGTEMPLATE['FOOTER'],footer_file)
